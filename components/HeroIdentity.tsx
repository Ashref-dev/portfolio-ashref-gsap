...
export const HeroIdentity = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      // Kill the CSS animation immediately when GSAP takes over
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          pin: true,
          anticipatePin: 1,
          onEnter: () => gsap.to("body", { backgroundColor: "#171717", duration: 0.5 }),
          onLeave: () => gsap.to("body", { backgroundColor: "#ffffff", duration: 0.1 }), // Switch to light mode seamlessly
          onLeaveBack: () => gsap.to("body", { backgroundColor: "#fafafa", duration: 0.5 })
        }
      });

      // 1. Cursor expands vertically
      ...
      // 2. Shutters Open & Cursor Fades
      tl.to(".shutter-top", { yPercent: -100, duration: 1, ease: "power2.inOut" }, ">-0.2");
      tl.to(".shutter-bottom", { yPercent: 100, duration: 1, ease: "power2.inOut" }, "<");
      tl.to(cursorRef.current, { opacity: 0, duration: 0.2 }, "<");

      // 3. Content Reveal
      tl.fromTo(".grid-content", 
        { opacity: 0, y: 50 },
        { opacity: 1, y: 0, duration: 0.8, ease: "power2.out" },
        "-=0.5"
      );
      
      tl.from(".grid-item-text:not(.scale-word)", {
        y: 100, opacity: 0, stagger: 0.05, duration: 0.8, ease: "power3.out"
      }, "<");
      
      tl.from(scaleRef.current, {
        y: 150, opacity: 0, duration: 1, ease: "power3.out"
      }, "<0.1");

      // 4. THE TRANSITION
      // Fade out everything EXCEPT "SCALE"
      tl.to([".grid-item-text:not(.scale-word)", ".grid-item-aux"], {
        opacity: 0,
        y: -50,
        duration: 0.5,
        ease: "power2.in"
  ...
  return (
    <section ref={wrapperRef} className="relative h-screen w-full bg-neutral-900 text-white overflow-hidden z-20">
      ...
      <div ref={contentRef} className="relative w-full h-full flex items-center justify-center">
        ...
        {/* Cursor */}
        <div ref={cursorRef} className="absolute z-20 w-[2px] h-[60px] bg-white animate-pulse" />
        ...
        <div className="grid-content absolute inset-0 flex flex-col items-center justify-center w-full h-full p-4">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-12 mb-2 md:mb-6">
             <div className="grid-item-aux px-4 py-1.5 rounded-full border border-neutral-700 flex items-center gap-2 bg-neutral-800/50 backdrop-blur-sm">
               ...
               <span className="text-xs font-mono text-neutral-400 tracking-wider">INIT_SYSTEM</span>
             </div>
             
             <div className="flex flex-wrap justify-center gap-x-3 md:gap-x-6">
                {["I", "BUILD", "SMART", "SYSTEMS", "THAT"].map((word, i) => (
                  <span key={i} className="grid-item-text text-[5vw] md:text-[3vw] font-light tracking-tight leading-none">{word}</span>
                ))}
             </div>
          </div>
          ...
          <div className="relative z-0 leading-[0.8] flex justify-center items-center">
             <span ref={scaleRef} className="scale-word text-[25vw] font-black tracking-tighter text-white block origin-center">SCALE</span>
          </div>
